...
struct make_indices_impl<First, Step, N,
...
struct make_indices_
    : detail::make_indices_impl<First, Step,
...
public:
  ...
  template <class InputIterator,
            class = typename std::enable_if<
                std::is_same<T, typename std::iterator_traits<
                                    InputIterator>::value_type>::value>::type>
  ...
  constexpr const_reference at(size_type const n) const {
    return (n >= length_) ? throw std::out_of_range("array_view::at()")
                          : *(data_ + n);
  }
  ...
  constexpr array_view<T> slice(check_bound_t, size_type const pos,
  ...
  constexpr array_view<T> slice_before(check_bound_t,
                                       size_type const pos) const {
    return (pos >= length_) ? throw std::out_of_range("array_view::slice()")
                            : array_view<T>{begin(), begin() + pos};
  }
  constexpr array_view<T> slice_after(check_bound_t,
                                      size_type const pos) const {
    return (pos >= length_) ? throw std::out_of_range("array_view::slice()")
                            : array_view<T>{begin() + pos, end()};
  }

  constexpr array_view<T> slice(size_type const pos,
                                size_type const length) const {
  ...
  constexpr array_view<T> slice(check_bound_t, iterator start,
                                iterator last) const {
    return (start >= end() || last > end() || start > last ||
            static_cast<size_t>(
                std::distance(start, last > end() ? end() : last)) >
...
operator_equal_impl(ArrayL const &lhs, size_t const lhs_size, ArrayR const &rhs,
                    size_t const rhs_size, IterL const litr, IterR const ritr) {
  return (litr != std::end(lhs))
             ? (!(*litr == *ritr)
                    ? false
                    : operator_equal_impl(lhs, lhs_size, rhs, rhs_size,
                                          std::next(litr), std::next(ritr)))
             : true;
}
...
inline constexpr bool operator==(array_view<T1> const &lhs,
...
template <
    class T, class Array,
    class = typename std::enable_if<detail::is_array_class<Array>::value>::type>
...
template <class T, class Array,
          class = typename std::enable_if<is_array<Array>::value>::type>
inline constexpr bool operator!=(array_view<T> const &lhs, Array const &rhs) {
  return !(lhs == rhs);
}
...
template <class Array, class T,
          class = typename std::enable_if<is_array<Array>::value, Array>::type>
inline constexpr bool operator!=(Array const &lhs, array_view<T> const &rhs) {
  return !(rhs == lhs);
}

template <class Array, class = typename std::enable_if<
                           detail::is_array_class<Array>::value>::type>
inline constexpr auto make_view(Array const &a)
...
template <class T>
inline constexpr array_view<T>
make_view(T const *p, typename array_view<T>::size_type const n) {
  return array_view<T>{p, n};
}

template <class InputIterator,
          class Result = array_view<
